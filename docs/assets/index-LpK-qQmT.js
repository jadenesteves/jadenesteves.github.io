(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))r(i);new MutationObserver(i=>{for(const s of i)if(s.type==="childList")for(const a of s.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&r(a)}).observe(document,{childList:!0,subtree:!0});function t(i){const s={};return i.integrity&&(s.integrity=i.integrity),i.referrerPolicy&&(s.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?s.credentials="include":i.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function r(i){if(i.ep)return;i.ep=!0;const s=t(i);fetch(i.href,s)}})();class g{projects=[];container=null;constructor(){this.container=document.getElementById("projects-container")}async init(){if(this.container)try{await this.loadProjects(),this.renderProjects()}catch(e){console.error("Failed to load projects:",e),this.renderFallbackProjects()}}async loadProjects(){this.projects=this.getExampleProjects()}getExampleProjects(){return[{id:1,name:"FGC Event Finder",description:"Ultra-fast FGC event finder with SQLite caching and Google Sheets integration",html_url:"https://github.com/jadenesteves/fgc-event-finder",homepage:null,language:"JavaScript",languages_url:"",stargazers_count:0,forks_count:0,topics:["javascript","HTML","sqlite","google-sheets","fgc"],created_at:"2024-01-15T10:00:00Z",updated_at:"2024-12-01T15:30:00Z",languages:{JavaScript:85,other:15},image:"/images/fgceventfinder.webp",wip:!0},{id:2,name:"Uber Analytics 2024",description:"Data analytics project using Uber trip data for 2024",html_url:"https://github.com/jadenesteves/Uber-Ride-Analytics-2024",homepage:null,language:"Python",languages_url:"",stargazers_count:0,forks_count:0,topics:["python","data-analytics","uber","visualization"],created_at:"2024-01-15T10:00:00Z",updated_at:"2024-12-01T15:30:00Z",languages:{Python:85,Jupyter:15},image:"/images/uber-analytics.webp",wip:!0},{id:3,name:"Expense Tracker App",description:"Android mobile application used to track your expenses",html_url:"https://github.com/jadenesteves/ExpenseTrackerApp",homepage:null,language:"Java",languages_url:"",stargazers_count:0,forks_count:0,topics:["android","java","mobile","expense-tracker"],created_at:"2024-02-20T14:00:00Z",updated_at:"2024-11-15T09:45:00Z",languages:{Java:80,XML:20},image:"/images/expense-tracker.webp"},{id:4,name:"Senior Capstone Assignment",description:"Experiment on human body doubling vs virtual human body doubling",html_url:"https://github.com/ap26131/VH-Body-Doubling-for-Productivity-",homepage:null,language:"JavaScript",languages_url:"",stargazers_count:0,forks_count:0,topics:["research","experiment","body-doubling","virtual"],created_at:"2024-03-10T12:00:00Z",updated_at:"2024-11-20T16:20:00Z",languages:{JavaScript:60,HTML:25,CSS:15},image:"/images/capstone.webp"},{id:5,name:"Project Lightfall: Top-Down Prototype",description:"Top-down horror game prototype made in Gamemaker Studio",html_url:"https://github.com/jadenesteves/Project-Lightfall-Top-Down-Prototype",homepage:null,language:"GML",languages_url:"",stargazers_count:0,forks_count:0,topics:["gamemaker","horror","game-development","prototype"],created_at:"2024-04-05T09:00:00Z",updated_at:"2024-12-01T11:10:00Z",languages:{GML:90,Other:10},image:"/images/lightfall.webp"}]}renderProjects(){this.container&&(this.container.innerHTML=this.projects.map(e=>this.createProjectCard(e)).join(""))}renderFallbackProjects(){this.container&&(this.container.innerHTML=`
    ...
    `)}createProjectCard(e){const t={TypeScript:"#3178c6",JavaScript:"#f7df1e",Python:"#3776ab",CSS:"#1572b6",HTML:"#e34c26",Java:"#ed8b00","C++":"#00599c",Go:"#00add8",GML:"#8fb010"},r=e.language||"Unknown",i=t[r]||"#6b7280";return`
      <div class="project-card fade-in ${e.wip?"project-wip":""}">
        ...
        <div class="project-content">
          ...
          <div class="project-tech">
            ...
            ${e.language?`<span class="tech-tag" style="background-color: ${i}; color: white;">${e.language}</span>`:""}
          </div>
    ...
    `,document.head.appendChild(e)}async handleFormSubmission(){if(!this.form||!this.submitButton)return;const e=this.getFormData();if(this.validateForm(e)){this.setSubmitButtonState(!0,"Sending...");try{await this.submitForm(e),this.showSuccessMessage(),this.resetForm()}catch(t){console.error("Form submission error:",t),this.showErrorMessage("Failed to send message. Please try again.")}finally{this.setSubmitButtonState(!1,"Send Message")}}}getFormData(){if(!this.form)throw new Error("Form not found");const e=new FormData(this.form);return{name:e.get("name")||this.form.querySelector('input[type="text"]')?.value||"",email:e.get("email")||this.form.querySelector('input[type="email"]')?.value||"",message:e.get("message")||this.form.querySelector("textarea")?.value||""}}validateForm(e){let t=!0;e.name.trim()?e.name.trim().length<2&&(this.showFieldError("name","Name must be at least 2 characters"),t=!1):(this.showFieldError("name","Name is required"),t=!1);const r=/^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/;return e.email.trim()?r.test(e.email)||(this.showFieldError("email","Please enter a valid email address"),t=!1):(this.showFieldError("email","Email is required"),t=!1),e.message.trim()?e.message.trim().length<10&&(this.showFieldError("message","Message must be at least 10 characters"),t=!1):(this.showFieldError("message","Message is required"),t=!1),t}validateField(e){const t=e.value.trim(),r=e.type||e.tagName.toLowerCase();switch(this.clearFieldError(e),r){case"text":t?t.length<2&&this.showFieldError("name","Name must be at least 2 characters"):this.showFieldError("name","Name is required");break;case"email":t?/^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/.test(t)||this.showFieldError("email","Please enter a valid email address"):this.showFieldError("email","Email is required");break;case"textarea":t?t.length<10&&this.showFieldError("message","Message must be at least 10 characters"):this.showFieldError("message","Message is required");break}}showFieldError(e,t){const r=this.getFieldByName(e);if(!r)return;r.classList.add("form-error");const i=r.parentElement?.querySelector(".error-message");i&&i.remove();const s=document.createElement("span");s.className="error-message",s.textContent=t,r.parentElement?.appendChild(s)}clearFieldError(e){e.classList.remove("form-error");const t=e.parentElement?.querySelector(".error-message");t&&t.remove()}getFieldByName(e){if(!this.form)return null;let t=this.form.querySelector(`[name="${e}"]`);if(!t)switch(e){case"name":t=this.form.querySelector('input[type="text"]');break;case"email":t=this.form.querySelector('input[type="email"]');break;case"message":t=this.form.querySelector("textarea");break}return t}async submitForm(e){await new Promise(t=>setTimeout(t,2e3)),console.log("Form submitted:",e)}setSubmitButtonState(e,t){this.submitButton&&(this.submitButton.disabled=e,this.submitButton.textContent=t,e?this.submitButton.style.opacity="0.7":this.submitButton.style.opacity="1")}showSuccessMessage(){const e=this.form?.parentElement?.querySelector(".form-success");e&&e.remove();const t=document.createElement("div");t.className="form-success",t.innerHTML=`
      <strong>Message sent successfully!</strong><br>
      Thank you for reaching out. I'll get back to you soon.
    `,this.form?.parentElement?.insertBefore(t,this.form),setTimeout(()=>{t.remove()},5e3)}showErrorMessage(e){alert(e)}resetForm(){if(!this.form)return;this.form.reset(),this.form.querySelectorAll("input, textarea").forEach(t=>{this.clearFieldError(t)})}}class f{projectsManager;skillsManager;resumeManager;contactManager;constructor(){this.projectsManager=new g,this.skillsManager=new u,this.resumeManager=new h,this.contactManager=new v}async init(){this.setupSmoothScrolling(),await this.projectsManager.init(),await this.skillsManager.init(),await this.resumeManager.init(),this.contactManager.init(),this.setupAnimations(),console.log("Portfolio app initialized successfully!")}setupSmoothScrolling(){document.querySelectorAll('.nav-menu a[href^="#"]').forEach(t=>{t.addEventListener("click",r=>{r.preventDefault();const i=t.getAttribute("href");if(i){const s=document.querySelector(i);if(s){const a=s.offsetTop;window.scrollTo({top:a,behavior:"smooth"})}}})}),this.setupScrollSpy()}setupScrollSpy(){const e=document.querySelectorAll("section"),t=document.querySelectorAll(".nav-section"),r=()=>{const i=window.scrollY+100,s=window.innerHeight,a=document.documentElement.scrollHeight,d=window.scrollY+s>=a-10;let c="";d?c=e[e.length-1].id:e.forEach(n=>{const l=n.offsetTop,m=n.offsetHeight,p=n.id;i>=l&&i<l+m&&(c=p)}),t.forEach(n=>{n.getAttribute("data-section")===c?n.classList.add("active"):n.classList.remove("active")})};r(),window.addEventListener("scroll",r,{passive:!0})}setupAnimations(){const e={threshold:.1,rootMargin:"0px 0px -50px 0px"},t=new IntersectionObserver(i=>{i.forEach(s=>{s.isIntersecting&&s.target.classList.add("fade-in")})},e);document.querySelectorAll("section").forEach(i=>{t.observe(i)})}}function y(){const o=document.getElementById("loading-spinner"),e=document.getElementById("layout-container");o&&(o.style.display="none"),e&&(e.style.opacity="1",e.classList.add("loaded"))}setTimeout(y,3e3);document.addEventListener("DOMContentLoaded",async()=>{try{await new f().init(),"fonts"in document&&await document.fonts.ready,await new Promise(r=>setTimeout(r,100));const e=document.getElementById("loading-spinner"),t=document.getElementById("layout-container");e&&t&&(e.style.display="none",t.classList.add("loaded"))}catch(o){console.error("Error initializing portfolio app:",o);const e=document.getElementById("loading-spinner"),t=document.getElementById("layout-container");e&&t&&(e.style.display="none",t.classList.add("loaded"));const r=document.querySelector("#about .hero-content");if(r){const i=document.createElement("div");i.style.cssText="background: #ff4444; color: white; padding: 1rem; border-radius: 8px; margin: 1rem 0; text-align: center;",i.innerHTML="⚠️ Some interactive features may not be fully loaded. The site is still functional!",r.appendChild(i)}}});
